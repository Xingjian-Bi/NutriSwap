...
export const getUserProfile = async (req, res) => {
  ...
  try {
    ...
    if (user.profile) {
      return res.status(200).json({ message: "Profile found", profile: user.profile });
    }

    // If no profile exists, prompt the client to fill in profile details
    return res.status(200).json({ message: "No profile found. Please complete your profile.", profile: null });
  } catch (error) {
